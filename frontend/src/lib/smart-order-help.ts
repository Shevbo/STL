...
export function preview(p: PreviewInput): Preview {
  ...
  if (tpTrail && !tp) error = error || 'Следящий тейк: укажите уровень активации (тейк, п.) больше нуля.';
  ...
  if (sl || tp || tra) {
    const parts = [sl ? `стоп ${fmtPts(sl)}` : '', tra ? `подтягивающая ${fmtPts(tra)}` : '',
      tp ? (tpTrail ? `следящий тейк: активация через ${fmtPts(tp)}, откат ${fmtPts(tpTrail)}`
                    : `тейк ${fmtPts(tp)}`) : ''].filter(Boolean).join(' и ');
