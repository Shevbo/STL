...
describe('rich_fool: описание на портале', () => {
  it('у страницы есть имя и hero-описание (не пустой hero)', () => {
    ...
    // __inv — отдельный hero: стороны перевёрнуты, а не тот же текст
    expect(overviewFor('rich_fool__inv')?.entry).toContain('фейдит');
    expect(overviewFor('rich_fool__inv')?.entry).not.toBe(overviewFor('rich_fool')?.entry);
  });
  ...
  it('уровни считаются убывающим шагом 1/(n+1), а не линейно', () => {
    const t = behaviorFor('rich_fool', defaults)!;
    expect(t).toContain('уровни 0.50, 0.83, 1.08');
    // мёртвых ключей расстановки в тексте больше нет
