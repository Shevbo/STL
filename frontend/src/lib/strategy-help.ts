...
const SHARED: Record<string, ParamHelp> = {
  avg_max: {
    title: 'Усреднение: потолок доборов',
    short: 'Потолок ТОЛЬКО лестницы усреднения (не всей позиции)',
    what: 'Ограничивает лишь контракты, ДОБРАННЫЕ усреднением против движения — и работает ТОЛЬКО когда включено усреднение (avg_step_atr > 0). Это НЕ абсолютный потолок позиции: даже при avg_max=1 ставочная система (bet_step/bet_max) наращивает сам вход, поэтому позиция может дойти до qty + bet_max контрактов. Абсолютный потолок = qty + bet_max.',
    how: 'Докупка идёт шагами, пока |позиция| < avg_max, и только при avg_step_atr > 0. Реальный максимум контрактов задаёт bet_max, а не avg_max — сверяйся по нему с лимитами агента и ГО.',
    schematic: 'ladder',
  },
  avg_step_atr: {
...
export const STRATEGY_HELP: Record<string, Record<string, ParamHelp>> = {
  fvg: {
    ...
    min_frac: {
      ...
      schematic: 'fvg',
      live: (v, c) => (c.price > 0) ? `тело ≥ ${pts(c.price * v / 10000)} при цене ${c.price.toLocaleString('ru-RU', { maximumFractionDigits: 6 })} (${(v / 100).toFixed(2)}%)` : null,
    },
    ...SHARED,
  },
};
...
const GENERIC: Record<string, ParamHelp> = {
  ...
  impulse_frac: { title: 'Порог импульса (×10000)', short: 'Мин. тело импульсной свечи',
    ...
    live: (v, c) => c.price > 0 ? `тело импульса ≥ ${(c.price * v / 10000).toLocaleString('ru-RU', { maximumFractionDigits: 6 })} п. (${(v / 100).toFixed(2)}%)` : null },
...
export const STRATEGY_OVERVIEW: Record<string, StrategyOverview> = {
  ...
  rich_fool: {
    timeframe: 'Один заход в день, привязанный к ОТКРЫТИЮ ТОРГОВ. Заявки выставляются ДО открытия — за place_lead_min минут до него — и живут hold_min минут после. Конкретный час нигде не задан: он берётся из самих данных (открытие — первый бар дня, закрытие — последний бар предыдущего дня того же типа), потому что расписание FORTS менялось внутри истории: у июньского контракта RI будни открывались в 09:00, у сентябрьского — уже в 07:00. Выходные FORTS торгует отдельной сессией, с 10:00 до 19:00. Сигнал считается по минутным барам.',
    entry: 'Предоткрытийная лестница ФЕЙДА импульса. От вчерашнего закрытия вверх и вниз расставляются по step_count заявок по формуле price(n) = price(n-1) + D/(n+1), где D = amp × d_coef: amp — средний дневной размах high-low за n_days, а d_coef от 0.2 до 1.0 СУЖАЕТ лестницу, придвигая её к цене. Зазоры УБЫВАЮТ (D/2, D/3, D/4, …), то есть ступени стоят гуще по мере удаления. Пример при закрытии 81000, amp 2500 и d_coef 1.0: 82250, 83083, 83708. Робот не следует за импульсом, а принимает противоположную сторону: уровни ВЫШЕ вчерашнего закрытия — это заявки на ПРОДАЖУ (лимитные, встречают цену сверху), уровни НИЖЕ — на ПОКУПКУ. Цена ускакала ВВЕРХ — открывается ШОРТ и добирается на каждой следующей ступени вверх; ускакала ВНИЗ — ЛОНГ с добором по ступеням вниз. Объём задаётся ОДНИМ множителем vol_mult: каждая следующая заявка во столько же раз крупнее предыдущей (при 1.0 — все равны), с потолком max_contracts. Первое срабатывание запирает сторону: встречные ступени до конца дня мертвы. Вариант «(контр)» (invert=1) — контрольный прогон с обратными сторонами.',
    tp: 'Тейк — ТРЕЙЛИНГ. Робот запоминает экстремум, достигнутый В ЕГО ПОЛЬЗУ (минимум для шорта, максимум для лонга), и закрывает позицию, когда цена откатывает от этого экстремума на trail_tp_pct % ОТ ЦЕНЫ. Трейлинг срабатывает ТОЛЬКО в прибыли: откат не имеет права закрыть позицию хуже входа — это работа стопа. Экстремум учитывается со СЛЕДУЮЩЕГО бара: один бар не может и задать пик своим минимумом, и выбить по нему тейк своим максимумом, порядок тиков внутри бара неизвестен. ОВЕРНАЙТ ЗАПРЕЩЁН — каждое утро нужна новая лестница, поэтому позиция не ночует: за exit_lead_min минут до закрытия включается выход по сигналу двух EMA (ema_fast/ema_slow) против позиции, на закрытии сессии позиция закрывается принудительно, а если день почему-то кончился раньше ожидаемого, остаток снимается на первом баре следующего дня.',
    sl: 'Стоп считается от СРЕДНЕЙ цены позиции и ставится ЗА ПРЕДЕЛАМИ всех заявок лестницы — дальше последней ступени, плюс запас sl_price_pct % от цены. Иначе стоп выбивал бы позицию раньше, чем лестница успевает добрать, и число ступеней было бы бутафорией. Средняя двигается с каждым добором, вместе с ней двигается и стоп. Внутри бара приоритет у стопа: накрыл бар и стоп, и тейк — засчитывается стоп. Бар, открывшийся гэпом за стопом, исполняется по цене ОТКРЫТИЯ, с проскальзыванием. Усреднения ПРОТИВ движения нет — лестница добирает только в ту сторону, куда ушла цена.'
  },
};

// Контр-версия (invert=1): тот же сетап, но стороны перевёрнуты — пробой фейдится.
STRATEGY_OVERVIEW['rich_fool__inv'] = {
  ...STRATEGY_OVERVIEW['rich_fool'],
  entry: 'КОНТРОЛЬНЫЙ вариант (invert=1) с ОБРАТНЫМИ сторонами: здесь робот следует за импульсом, а не фейдит его — цена ускакала ВВЕРХ значит ЛОНГ, ВНИЗ значит ШОРТ. Нужен как зеркало: рабочий фейд обязан его обыгрывать на ТЕХ ЖЕ параметрах, иначе преимущества нет, а есть подгонка. Лестница та же: от вчерашнего закрытия step_count уровней по формуле price(n) = price(n-1) + D/(n+1), D = amp × d_coef, зазоры убывают.',
};

export function overviewFor(strategyId: string): StrategyOverview | null {
  return STRATEGY_OVERVIEW[strategyId] ?? STRATEGY_OVERVIEW[strategyId.replace(/__inv$/, '')] ?? null;
}
...
const STRATEGY_COPY_BY: Record<string, StrategyCopyBy> = {
  rich_fool: { author: 'claude opus 5', date: 'сентябрь 2026' },
};
...
export const STRATEGY_NAME: Record<string, string> = {
  ...
  rich_fool: 'Rich Fool — лестница фейда импульса',
};
...
export function behaviorFor(strategyId: string, params: Record<string, any> | null | undefined, symbol?: string): string | null {
  ...
  if (strategyId === 'rich_fool' || strategyId === 'rich_fool__inv') {
    ...
    const dCoef = num('d_coef', 100) / 100;            // 20..100 -> 0.20..1.00
    const slPct = num('sl_price_pct', 100) / 100;      // ×100 -> проценты
    const trailPct = num('trail_tp_pct', 50) / 100;
    const volMult = num('vol_mult', 10) / 10;
    const maxC = Math.max(1, Math.round(num('max_contracts', 100)));
    const exitLead = Math.max(0, Math.round(num('exit_lead_min', 120)));
    const emaF = Math.max(2, Math.round(num('ema_fast', 9)));
    const emaS = Math.max(3, Math.round(num('ema_slow', 21)));
    const invert = num('invert', 0) === 1 || strategyId.endsWith('__inv');
    // Смещение ступени k = D·Σ1/(j+1), j=1..k, где D = amp·d_coef.
    const offs: number[] = [];
    let acc = 0;
    for (let j = 1; j <= steps; j++) { acc += dCoef / (j + 1); offs.push(acc); }
    const shown = offs.slice(0, 4).map((o) => o.toFixed(2)).join(', ')
      + (steps > 4 ? ` … (всего ${steps})` : '');
    const upSide = invert ? 'ЛОНГ' : 'ШОРТ';
    const dnSide = invert ? 'ШОРТ' : 'ЛОНГ';
    const sideTxt = invert
      ? `Это КОНТРОЛЬНЫЙ вариант (invert=1): робот следует за импульсом, а не фейдит его.`
      : `Робот ФЕЙДИТ импульс: продаёт то, что улетело вверх, и покупает то, что улетело вниз.`;
    const volTxt = volMult !== 1
      ? `, причём каждая следующая заявка в ${volMult.toFixed(1)} раза крупнее предыдущей`
      : ', объём всех ступеней одинаковый';
    const exitTxt = exitLead > 0
      ? `За ${exitLead} мин до закрытия включается выход по двум EMA (${emaF}/${emaS}) против позиции`
      : 'Выход по двум EMA отключён';
    return `За ${lead} мин ДО ОТКРЫТИЯ ТОРГОВ строит от вчерашнего закрытия ${sym} две лестницы по ${steps} уровн. на УБЫВАЮЩЕМ шаге: уровни ${shown} амплитуды, где амплитуда — средний дневной размах за ${nDays} дн., сжатый коэффициентом d_coef=${dCoef.toFixed(2)}. Цена ушла ВВЕРХ — робот берёт ${upSide} на ${num('qty', 1)} контракт(ов) и добирает на каждой следующей ступени вверх; ушла ВНИЗ — ${dnSide} с добором по ступеням вниз${volTxt}, потолок ${maxC} контракт(ов). ${sideTxt} Один бар, прошивший несколько уровней, исполняет их все сразу; первое срабатывание запирает сторону. Через ${hold} мин после открытия неисполненные заявки снимаются — но только если не было НИ ОДНОЙ сделки; если лестница начала набирать, она продолжает работать. Стоп стоит ЗА последней ступенью лестницы плюс ${slPct.toFixed(2)}% от средней цены позиции; тейк — трейлинг, откат на ${trailPct.toFixed(2)}% от лучшего достигнутого уровня, только в прибыли. Овернайта нет: ${exitTxt}, на закрытии сессии позиция закрывается принудительно.`;
  }
