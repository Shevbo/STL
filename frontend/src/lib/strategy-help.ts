...
export const STRATEGY_OVERVIEW: Record<string, StrategyOverview> = {
  ...
  rich_fool: {
    ...
    entry: 'Предоткрытийная лестница пробоя. От вчерашнего закрытия вверх и вниз расставляются step_count стоп-заявок по формуле price(n) = price(n-1) + D/(n+1), где D — средний дневной размах high-low за n_days: зазоры УБЫВАЮТ (D/2, D/3, D/4, …), а не равны. Пример при закрытии 81000 и D = 2500: 82250, 83083, 83708. Сторону решает факт срабатывания: пробило вверх — робот в лонге, вниз — в шорте. invert=1 переворачивает обе стороны (фейд пробоя).',
