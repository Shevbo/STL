...
export const STRATEGY_OVERVIEW: Record<string, StrategyOverview> = {
  ...
  rich_fool: {
    ...
    entry: 'Предоткрытийная лестница пробоя. От вчерашнего закрытия вверх и вниз расставляются step_count стоп-заявок по формуле price(n) = price(n-1) + D/(n+1), где D — средний дневной размах high-low за n_days: зазоры УБЫВАЮТ (D/2, D/3, D/4, …), а не равны. Пример при закрытии 81000 и D = 2500: 82250, 83083, 83708. Направление заранее не выбирается: позицию открывает та стоп-заявка, до которой цена дошла первой — верхняя ступень это лонг, нижняя шорт. invert=1 меняет стороны местами (фейд: пробой вверх продаёт, вниз покупает).',
    tp: 'Тейк — в долях риска: rr_x10/10 × R (по умолчанию 2:1), от средней входа. Один бар может прошить сразу несколько уровней — исполняются ВСЕ пересечённые ступени в том же баре, а не только первая. Незаполненные ступени снимаются через hold_min минут после открытия. Позиция НЕ закрывается принудительно в конце дня — держится до тейка или стопа, в том числе овернайт.',
...
STRATEGY_OVERVIEW['rich_fool__inv'] = {
  ...
  entry: 'Предоткрытийная лестница пробоя, но СТОРОНЫ ПЕРЕВЁРНУТЫ (invert=1): пробой вверх робот продаёт, пробой вниз покупает — он фейдит пробой, а не следует ему. Ступени те же: от вчерашнего закрытия step_count стоп-заявок по формуле price(n) = price(n-1) + D/(n+1) (зазоры D/2, D/3, D/4, …), D — средний дневной размах high-low за n_days.',
};
...
export function behaviorFor(strategyId: string, params: Record<string, any> | null | undefined, symbol?: string): string | null {
  ...
  if (strategyId === 'rich_fool' || strategyId === 'rich_fool__inv') {
    ...
    const ladderTxt = `${steps} ступеней вверх и ${steps} вниз на убывающем шаге от закрытия: уровни ${offs.map(o => o.toFixed(2)).join(', ')} амплитуды`;
    const sideTxt = invert
      ...
      : `Направление задаёт первая сработавшая ступень: верхняя — покупка, нижняя — продажа.`;
    const volTxt = volMult !== 10
      ? `, объём каждой следующей ступени той же стороны умножается на ${(volMult / 10).toFixed(1)}`
      : '';
    return `За ${lead} мин до открытия (${openTxt} МСК) ставит от вчерашнего закрытия ${sym} ${ladderTxt} стоп-заявок. Амплитуда — средний дневной размах за ${nDays} дн. Заявки живут ${hold} мин после открытия, неисполненные снимаются. ${sideTxt} Один бар, прошивший несколько уровней, исполняет их все сразу. Сработавшая ступень — вход на ${num('qty', 1)} контракт(ов); следующие ступени той же стороны в пределах окна доливают${volTxt}, потолок ${maxC} контракт(ов). Выход ЖЁСТКИЙ с обеих сторон: стоп ${(slPct / 100).toFixed(2)} амплитуды от средней, тейк ${(rr / 10).toFixed(1)} к 1 от стопа. В конце дня позиция НЕ закрывается — носится овернайт до тейка или стопа.`;
  }
