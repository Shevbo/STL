...
export const STRATEGY_OVERVIEW: Record<string, StrategyOverview> = {
  ...
  rich_fool: {
    timeframe: 'Один заход в день. Заявки выставляются ДО открытия биржи — за place_lead_min минут до него (якорь открытия задаёт open_hour, для утренней сессии FORTS это 07:00 МСК) — и живут hold_min минут после открытия, неисполненные снимаются. В этом вся суть: лестница стоит заранее и ловит открытие, а не строится по ходу дня. Сигнал считается по минутным барам; открывшаяся позиция живёт дальше окна — до выхода, в том числе через ночь.',
    entry: 'Предоткрытийная лестница ФЕЙДА импульса. От вчерашнего закрытия вверх и вниз расставляются по step_count заявок по формуле price(n) = price(n-1) + D/(n+1), где D — средний дневной размах high-low за n_days: зазоры УБЫВАЮТ (D/2, D/3, D/4, …), а не равны. Пример при закрытии 81000 и D = 2500: 82250, 83083, 83708. Робот ФЕЙДИТ импульс, а не следует за ним: уровни ВЫШЕ вчерашнего закрытия — это заявки на ПРОДАЖУ (лимитные, встречают цену сверху), уровни НИЖЕ — на ПОКУПКУ. Цена ускакала ВВЕРХ — открывается ШОРТ и добирается на каждой следующей ступени вверх; ускакала ВНИЗ — ЛОНГ с добором по ступеням вниз. Объём ступеней задаётся vol_mult: при 1.0 все ступени по qty контрактов, при большем значении каждая следующая во столько же раз крупнее предыдущей, с общим потолком max_contracts. Первое срабатывание запирает сторону: встречные ступени до конца дня мертвы. Вариант «(контр)» (invert=1) — контрольный прогон с обратными сторонами.',
    tp: 'Тейк — ТРЕЙЛИНГ. Робот запоминает экстремум, достигнутый В ЕГО ПОЛЬЗУ (минимум для шорта, максимум для лонга), и закрывает позицию, когда цена откатывает от этого экстремума на trail_tp_pct % ОТ ЦЕНЫ. Трейлинг срабатывает ТОЛЬКО в прибыли: откат не имеет права закрыть позицию хуже входа — это работа стопа. Экстремум учитывается со СЛЕДУЮЩЕГО бара: один бар не может и задать пик своим минимумом, и выбить по нему тейк своим максимумом — порядок тиков внутри бара неизвестен. Один бар, прошивший несколько уровней входа, исполняет их ВСЕ сразу. Незаполненные ступени снимаются через hold_min минут. В конце дня позиция принудительно НЕ закрывается — держится до тейка или стопа, в том числе овернайт.',
    sl: 'Стоп — СТАНДАРТНЫЙ, в процентах ОТ ЦЕНЫ: расстояние = средняя цена входа × sl_price_pct %, на весь объём. Считается от СРЕДНЕЙ, поэтому каждый добор по лестнице сдвигает стоп вместе со средней. Внутри бара приоритет у стопа: накрыл бар и стоп, и тейк — засчитывается стоп. Бар, открывшийся гэпом за стопом, исполняется по цене ОТКРЫТИЯ, с проскальзыванием, а не по уровню. Усреднения ПРОТИВ движения нет — лестница добирает только в ту сторону, куда ушла цена. Важно понимать конкуренцию двух расстояний: зазор до второй ступени на RI около 1% цены, поэтому при тесном стопе позиция закрывается раньше, чем успевает добрать.'
  },
};
...
STRATEGY_OVERVIEW['rich_fool__inv'] = {
  ...
  entry: 'КОНТРОЛЬНЫЙ вариант (invert=1) с ОБРАТНЫМИ сторонами: здесь робот следует за импульсом, а не фейдит его — цена ускакала ВВЕРХ значит ЛОНГ, ВНИЗ значит ШОРТ. Нужен как зеркало: рабочий фейд обязан его обыгрывать, иначе преимущества нет, а есть подгонка. Ступени те же: от вчерашнего закрытия step_count уровней по формуле price(n) = price(n-1) + D/(n+1) (зазоры D/2, D/3, D/4, … убывают), D — средний дневной размах high-low за n_days.',
};
...
export function behaviorFor(strategyId: string, params: Record<string, any> | null | undefined, symbol?: string): string | null {
  ...
  if (strategyId === 'rich_fool' || strategyId === 'rich_fool__inv') {
    ...
    const slPct = num('sl_price_pct', 100) / 100, trailPct = num('trail_tp_pct', 50) / 100;
    ...
    // price(n) = price(n-1) + amp/(n+1) → смещение ступени k = amp·Σ 1/(j+1), j=1..k
    const offs: number[] = [];
    let acc = 0;
    for (let j = 1; j <= steps; j++) { acc += 1 / (j + 1); offs.push(acc); }
    const shown = offs.slice(0, 4).map(o => o.toFixed(2)).join(', ') + (steps > 4 ? ` … (всего ${steps})` : '');
    const ladderTxt = `две лестницы по ${steps} уровн. на УБЫВАЮЩЕМ шаге от закрытия: ${shown} амплитуды`;
    ...
    const volTxt = volMult !== 10
      ? `, причём объём каждой следующей ступени умножается на ${(volMult / 10).toFixed(1)}`
      : ', объём всех ступеней одинаковый';
    return `За ${lead} мин до открытия (${openTxt} МСК) строит от вчерашнего закрытия ${sym} ${ladderTxt}, где амплитуда — средний дневной размах за ${nDays} дн. Цена ушла ВВЕРХ — робот берёт ${upSide} на ${num('qty', 1)} контракт(ов) и добирает на каждой следующей ступени вверх; ушла ВНИЗ — ${dnSide} с добором по ступеням вниз${volTxt}, потолок ${maxC} контракт(ов). ${sideTxt} Один бар, прошивший несколько уровней, исполняет их все сразу; первое срабатывание запирает сторону. Заявки живут ${hold} мин после открытия, неисполненные снимаются. Выход: стоп ${slPct.toFixed(2)}% ОТ ЦЕНЫ от средней входа (приоритетен внутри бара, гэп — по открытию) и ТРЕЙЛИНГ-тейк — откат на ${trailPct.toFixed(2)}% от лучшего достигнутого уровня, только в прибыли. В конце дня позиция НЕ закрывается — носится овернайт до стопа или тейка.`;
  }
