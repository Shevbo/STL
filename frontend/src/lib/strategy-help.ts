...
export const STRATEGY_OVERVIEW: Record<string, StrategyOverview> = {
  ...
  rich_fool: {
    timeframe: 'Один заход в день, привязанный к открытию сессии. Лестница выставляется за place_lead_min минут до открытия (рабочий режим — 10:00 МСК, основная сессия) и живёт hold_min минут после него. Утренняя сессия 07:00 в истории разрежена, по ней перебор почти не даёт сделок. Сигнал считается по минутным барам; открывшаяся позиция живёт дальше окна — до выхода, в том числе через ночь.',
    entry: 'Предоткрытийная лестница пробоя. От вчерашнего закрытия вверх и вниз расставляются step_count стоп-заявок по формуле price(n) = price(n-1) + D/(n+1), где D — средний дневной размах high-low за n_days: зазоры УБЫВАЮТ (D/2, D/3, D/4, …), а не равны. Пример при закрытии 81000 и D = 2500: 82250, 83083, 83708. Робот ФЕЙДИТ импульс, а не следует за ним: уровни ВЫШЕ закрытия — это заявки на ПРОДАЖУ, уровни НИЖЕ — на ПОКУПКУ. Цена ускакала ВВЕРХ — открывается ШОРТ и добирается на каждой следующей ступени вверх; ускакала ВНИЗ — ЛОНГ с добором по ступеням вниз. Объём ступени растёт: ступень hit = qty×(vol_mult/10)^hit, потолок max_contracts — чем дальше ушла цена, тем крупнее добор. Первое срабатывание запирает сторону: встречные ступени до конца дня мертвы. Вариант «(контр)» (invert=1) — контрольный прогон с обратными сторонами.',
...
export function behaviorFor(strategyId: string, params: Record<string, any> | null | undefined, symbol?: string): string | null {
  ...
  if (strategyId === 'rich_fool' || strategyId === 'rich_fool__inv') {
    ...
    const oh = Math.max(0, Math.round(num('open_hour', 10)));
