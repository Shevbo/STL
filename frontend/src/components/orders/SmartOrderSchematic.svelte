...
<figure class="so-schem" style="--accent:{meta.color}">
  ...
  {:else if kind === 'trail_sl'}
    <!-- Позиция УЖЕ открыта, поэтому сторожа никто не будит: уровень стоит с
         первого тика и едет только в сторону уменьшения убытка. -->
    <svg viewBox="0 0 320 150" role="img" aria-label="Схема подтягивающей">
      <g transform={flip ? 'translate(0,150) scale(1,-1)' : ''}>
        <path class="px" d="M14 116 L54 100 L92 106 L130 72 L168 78 L206 44 L232 52 L258 42 L300 92" />
        <path class="trail" d="M14 136 L54 120 L92 120 L130 92 L168 92 L206 64 L232 64 L258 62 L300 62" />
        <circle class="peak" cx="258" cy="42" r="4.5" />
        <circle class="hit" cx="294" cy="62" r="5" />
        <line class="gap" x1="258" y1="42" x2="258" y2="62" />
      </g>
      <text class="lbl" x="188" y={flip ? 128 : 36}>лучшая цена</text>
      <text class="lbl2" x="14" y={flip ? 22 : 148}>уровень выхода едет только вверх по прибыли</text>
      <text class="hitlbl" x="292" y={flip ? 108 : 80} text-anchor="end">здесь выход</text>
    </svg>
    <figcaption>
      Позиция уже открыта, поэтому уровень стоит <b>с первого тика</b>: активации
      у этого типа нет. Он держится в <b>{trailOffset > 0 ? fmtPts(trailOffset) : 'заданном отступе'}</b>
      от лучшей достигнутой цены и никогда не отступает назад — дошла цена, позиция закрыта.
    </figcaption>
