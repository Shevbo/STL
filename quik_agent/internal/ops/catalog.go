...
var Catalog = []Op{
	...
	// ГЛАВНАЯ диагностическая операция. 25.09.2026 терминал перестал отвечать в
	// 16:29 и торговля встала на семь часов: агент был жив, связь с STL зелёная,
	// а на вопрос «жив ли сам QUIK» ответить было нечем. Отвечает несколькими
	// независимыми методами разом, потому что каждый по отдельности врёт.
	{ID: "quik.health", Risk: RiskNone,
		Desc: "жив ли QUIK и быстро ли откликается: окно не отвечает, время отклика, CPU, возраст понга Lua"},
