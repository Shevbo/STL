...
const (
	...
	// Сотни хватало, пока связь цела (снимок уходит каждые пять секунд), но
	// 25.09.2026 агент был отключён от STL три часа при идущей торговле, и всё,
	// что не поместилось в хвост, для STL не существовало бы вовсе. В тот день
	// спас тихий рынок (85 сделок за сутки), а не механизм: 24.09 их было 265, и
	// такой же обрыв срезал бы начало дня безвозвратно.
	//
	// Хвост — это ВЕСЬ счёт, вместе с ручной торговлей оператора, поэтому мерить
	// его надо по самому оборотистому дню, а не по роботам. Пять тысяч строк это
	// единицы мегабайт JSON против невосстановимой дыры в учёте денег.
	quikTradesCap = 5000
	quikTransCap  = 50
	quikLinesCap  = 50
)
