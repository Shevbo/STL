...
**Доставка почты в живую сессию (12.08.2026).** Раньше письмо, пришедшее в
СЕРЕДИНЕ сессии, ты не видел до следующего запуска: будильник висел на SessionStart,
а сессия живёт десятками часов (11.08 письма лежали 7 и 30 часов при работающих
окнах). Теперь фон раз в 45 с кладёт твой ящик в локальный файл, а хук показывает
новое на каждом твоём вводе за ~48 мс и БЕЗ сети. Пустой ящик не печатает ничего.
Ставится в САМОМ окне (машина окон — Windows-VM рядом с vibe, снаружи в неё не
ходят), одна команда на окно; каждое окно запускается со своей переменной
`STL_WINDOW`:
...
Установщик сам разберётся, Windows это или Linux: планировщик задач или crontab,
раз в минуту. Демона нет намеренно.

Подробно: [docs/DEVMAIL.md](docs/DEVMAIL.md). Правила почты ниже не изменились —
`ack` обязателен, иначе письмо висит непрочитанным.
...
**ПРОСРОЧКА — это сбой службы, а не фон (11.08.2026).** Служба стояла на
допущении «адресат скоро откроется», и оно НИГДЕ не измерялось: `POST /msg`
отвечал `{ok, id}`, отправитель уходил считать задачу переданной, а письмо
лежало непрочитанным — на момент починки у `ui-ux` их было пять, старшему
30.6 часа. Узнал об этом человек, руками заглянув в ЧУЖОЙ ящик. Ни один софт
не может открыть окно Claude, поэтому починена не доставка, а СЛЕПОТА:
отправка возвращает состояние ящика адресата, возраст едет с каждым письмом
и с доской (`GET /api/v1/dev/board`, одним запросом), а письмо старше 4 часов
объявляется просроченным и печатается ЛЮБОМУ окну, которое читает свою почту.
Реакция на просрочку ровно одна: **сказать оператору**, он единственный, кто
откроет молчащее окно. Сделать чужую работу молча вместо эскалации ЗАПРЕЩЕНО —
это прячет сбой службы вместо того, чтобы его чинить. Будильник сессии теперь
ловит и `clear`: на `startup|resume` окно после `/clear` доску не видело вовсе.
