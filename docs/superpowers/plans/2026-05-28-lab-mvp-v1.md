...
Пошаговый план постройки лаборатории STL: модуля, который заводит роботов, гоняет
бэктесты на минутных барах и запускает бумажную и живую торговлю из одного интерфейса.
Отвечает на вопросы вида «из чего собирался LAB», «почему бэктест считается в отдельном
процессе, а не внутри API», «в каком порядке появлялись таблицы robots, backtest_runs и
live_trades». Относится к слою lab и backtest, к боевому исполнению заявок отношения не
имеет. В работе это называют лабораторией, стендом, песочницей стратегий.
