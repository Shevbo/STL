...
- [Хук почты окна](project_mail_hook.md) — mail_hook.py в settings.local.json: почта переживает перезапуск, только показ без автоответа (инцидент 18.08: 792 письма); на ответ/подтверждение не отвечать, только ack
...
- [flip_min_pts фильтр разворота](project_flip_min_pts.md) — macd_shectory1 инцидент 09.09; порог сатурирует на ~1400 из-за sl_pct=100; per-robot: помогает macdshort, ломает lxk22 за квартал
- [Райдеры и расписание сторон](project_riders_and_tod.md) — 14.08 подхват: tod_* в реестре, эскалация после убытка уже была; ждёт оператора арминг шортового двойника, выкладка фикса архива, GDU6 в MD_CODES; пуш из подхвата идёт по SSH
- [Проверять деньги субагентом](feedback_validate_money_with_fable.md) — STRICT: финансовые выводы отдавать fable «проверь мой вывод»; ann_return_go это ДОЛЯ (4.179 = 418% годовых), прочитал как 4.2% и забраковал хорошую строку
- [Направление, а не преимущество](reference_direction_not_edge.md) — живой конфиг: +325k/+426k на падении, −49k/−51k на росте, знак 4/4 против рынка; но убыток вшестнадцатеро меньше шорта той же вместимости; в отбор добавлены контроль «тупой шорт» и растущее окно
- [Вывод про ось из чужой строки](reference_confounded_axis.md) — строка лидерборда это ВЕКТОР: гипотеза «расписание вредит на росте» родилась из строки, отличной тремя параметрами, и была опровергнута чистым прогоном; сравнивать только matched pair
- [ВМ счёта = роботы + ручные](reference_vm_split_account.md) — ВМ QUIK это ДЕНЬ всего счёта с вечернего клиринга, валовый; ручную торговлю оператора не видел никто, теперь её считает блок day агента
- [Скидка MOEX за скальпинг](reference_commission_scalper_discount.md) — модель комиссии завышает сбор вдвое на оборотистых днях; у lxk22 это ~40k недостающих в итоге, ошибка наказывает высокочастотные стратегии
- [rich_fool: преимущества нет](project_rich_fool.md) — 900 векторов гейта на RIM6/RIU6 после time_exit, но вне выборки RIH6 0%, RIM5 −96k против зеркала +60k; бутстрап OOS P=0.57
- [Выводимый параметр в ключе гейта](reference_gate_derived_param_key.md) — d_coef поконтрактный в ключе сопоставления кварталов = гейт молча 0; rf7 остановлена по артефакту
- [Прокол impulse_fade ЗАКРЫТ](reference_impulse_fade_mirror.md) — 13.09 отказ: при реализуемом исполнении ни фейд, ни пробой; «переживший OOS» отбор (p<0.001) был артефактом исполнения, пять слоёв ловушек
- [Прогрев баров срезал историю](reference_warm_bars_clobber.md) — agent_bars/<контракт>.json пишут rf_fetch_contracts и cron warm_robot_bars 05:00; перезапись резала голову, проба расходилась с i9; починено слиянием 9fd8995
- [Филл внутри бара = заглядывание](reference_intrabar_fill_lookahead.md) — STRICT: у раннера нет стоп-заявок (вход пробоя = открытие следующего бара); гейт по cur.high при филле в том же баре — будущее; переживший OOS отбор ≠ эдж
- [Лидерборд за всё время: эджа нет](reference_leaderboard_alltime_no_edge.md) — windows 4/4 в БД = нарезка одного прогона; macd_shectory1 «5/5 окон» лонг-онли провалил вне выборки; 2EMA M1 и слои DeskBot закрыты (усреднение = плечо, сравнивать с базой равной экспозиции)
- [Золото bollinger_bo_m1 — не кандидат](reference_gd_bollinger_not_candidate.md) — лидер +697k провалил 4 нетронутых квартала роста (−0.94 млн ₽); истёкшие GD в пунктах ×84.26
- [Конфиг TSLab DeskBot 2EMA](reference_tslab_deskbot_config.md) — 2→64 закрыт прогоном самой TSLab 2022–2026 (итог ≈ −643k пт, плюс только в году оптимизации); фильтр волатильности провалил 2023; пила стопа при лестнице; RI.txt дополнен ISS с 2022
- [Хаос R и стопы на лидерах](reference_r_chaos_and_stops.md) — пользы нет; R у всегда-в-рынке = пропуск флипов; net без открытой позиции врёт — мерить MTM
- [Переключатель режимов и keltner ЗАКРЫТЫ](project_regime_switch_step0.md) — 14.09 M1 нетто все в минусе; 15.09 M15/H1 на RI гейт проходят 66% случайных трендовых (фактор), keltner_bo провалил BR и GD по пререгистрации; 2022-26 просмотрены, дальше только бумажный форвард
- [Бэктест по архивному стакану](project_book_replay.md) — BookRuntime: бар-исполнение оптимистично 6.6-19.3 пт/филл; платишь СРЕДНЕЕ 9-12 пт, медиана 5; ночные филлы 8-25% заявок и до 59% платы; ловушки: порог свежести снимка, среднее vs медиана
- [Смягчённый гейт: ожидание ноль](reference_soft_gate_rule.md) — «плюс на текущем окне» на 2022-2026: t 0.3-1.0, медиана окна в минусе; квинтили монотонны только снизу = фильтр мусора; плюс на GD объясняется тупым моментумом золота
- [Треугольник MX против RI×Si закрыт](reference_triangle_arb.md) — возврат реален ~95 ₽/круг, издержки ~200; валовый быстрого режима = bid-ask bounce (проверка задержкой на 1-2 бара); RI против Si не арбитраж
- [Стоп lxk22 — страховка хвоста](reference_lxk22_stop.md) — 2022-26: плюс только от ударов 2022, в спокойные годы не измерим, худшая просадка без стопа на треть глубже; боевая спека в robots-mirror, не в robots
- [Фильтр издержек cost_atr](project_cost_filter.md) — вход только если ATR >= порог × цены круга; на lxk22 порог 40 = «торгуй реже» (оставленные хуже убранных), отбор с 60, но плюс только из 2022; spread_pts=0 занижает порог вдвое
- [valley_spike ЗАКРЫТ](project_valley_spike.md) — перебор «дальнее × долина» 288 прогонов: 0 пар прошли гейт; частота есть только там, где минус; RIU6 плюс по режиму (обе стороны), не направлению; отказ проверен субагентом
- [Выход по флипу = издержки](reference_flip_exit_cost.md) — блок переворота помогает 12/12 клеток, но даёт ноль; на Si зарабатывает весь трендовый класс, плацебо не обойдено (p=0.117), доход около ставки
- [Считать только на i9](feedback_backtests_only_i9.md) — STRICT: бэктесты/переборы только на i9 через очередь; локально и на хостере не считать, агент запускает оператор
- [Лось от геометрии лестницы](reference_ladder_geometry_loss.md) — 21.09: причина не выходные/ATR (ось спасает 230 ₽ из 11 040), а шаг от СРЕДНЕЙ при k_avg=2: 20 контрактов в 2.77 шага; разножка 200 = пик 7 вместо 20
- [lib_sha и CRLF](reference_lib_sha_crlf.md) — отпечаток движка на i9 считается от LF-версии файла; локальный CRLF-файл даёт ложное «код не доехал»
- [Добор по уровню, а не шагом](reference_add_level_vs_step.md) — доборы на возврате цены = сетка от СРЕДНЕЙ (9.1% контрактов за 2 мес); ось avg_from_last нейтральна по деньгам, это не багфикс; ловушки двойного счёта и подгонки на дне-источнике
