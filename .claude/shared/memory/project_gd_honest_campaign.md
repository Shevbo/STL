...
Заметка о честных кампаниях перебора на i9: как ставились gdhonest по золоту и honest
по всему рынку, чем «честные» метрики отличаются от прежних и что тогда сломалось в
доставке кода на машину перебора. Отвечает на вопросы вида «почему кампанию считали
несколько суток», «как файлы попадают на i9, если raw.githubusercontent оттуда режется»,
«зачем останавливали оптимизатор и ключ pause_local». Слова, которыми это называют в
работе: кампания, перебор, сетка, джоба, воркер, лидерборд.
