...
class Bar:
    ...
    close: float
    volume: int
    # Была ли в этой минуте хоть одна РЕАЛЬНАЯ сделка с ленты. False = бар
    # синтезирован из котировок (`robot_runner/bars.py`: лента молчит >30 с,
    # строим из bid/ask/last — за ночь на стоящей бирже так набегают сотни
    # плоских минут). Дефолт True: бары бэктеста/ISS — это факт биржи.
    # Торговый путь этот флаг НЕ читает (стратегия видит бары как раньше);
    # он нужен потребителям, которым нельзя показывать выдуманную свечу.
    traded: bool = True
