...
# O(n) скользящими деками на весь хвост — урок pivot: никаких O(n×w) на каждом баре.
def dv_flags(closes: list, window: int, thr: float) -> list:
    """Пер-барные флаги «бар внутри долины»: True, когда окно из `window` последних
    закрытий (включая текущее) укладывается в коридор уже `thr` пунктов. Флаги
    исторических баров стабильны (зависят только от прошлого), поэтому сигнал,
    посчитанный без долинных баров, не «переигрывает» историю задним числом."""
    n = len(closes)
    flags = [False] * n
    if window <= 0 or thr <= 0 or n < window:
        return flags
    maxq: deque = deque()  # индексы, closes убывают
    minq: deque = deque()  # индексы, closes возрастают
    for i, c in enumerate(closes):
        while maxq and closes[maxq[-1]] <= c:
            maxq.pop()
        maxq.append(i)
        while minq and closes[minq[-1]] >= c:
            minq.pop()
        minq.append(i)
        lo = i - window + 1
        if lo < 0:
        ...
        while minq[0] < lo:
            minq.popleft()
        if closes[maxq[0]] - closes[minq[0]] < thr:
            flags[i] = True
    return flags


# Запас баров под хвостовую долину (8 часов М1): раннер всё равно ограничен своим
# хвостом, бэктест режет список — дорого не бывает.
DV_FETCH_EXTRA = 480


def dv_sig_window(bars, need: int, window: int, thr: float):
    """(sig_bars, in_dv) — сигнальная серия с вырезанной ХВОСТОВОЙ долиной.

    Вырезается ТОЛЬКО долина, в которой цена стоит прямо сейчас (непрерывный
    флагованный хвост): пока она длится, сигнал заморожен на до-долинном окне,
    а первый же небудничный бар возвращает СЫРУЮ серию целиком. Завершённые
    долины истории НЕ вырезаются. Первая версия вырезала все долинные бары за
    всю историю — и после ночной долины роботу не хватало чистых баров на
    прогрев ещё ~2 часа ПОСЛЕ пробоя (жив. lxk22, 07.08.2026: ход 600 пт, робот
    нем), а консоль (фильтр по всему хвосту) показывала живой сигнал — ровно
    рассинхрон стенда с движком, чиненный 05.08. Общий helper для make_on_bar И
    explain = паритет по построению."""
    flags = dv_flags([b.close for b in bars], window, thr)
    if not (flags and flags[-1]):
        return bars[-need:], False
    start = len(flags)
    while start > 0 and flags[start - 1]:
        start -= 1
    return bars[:start][-need:], True
...
def make_on_bar(rid: str):
    ...
    async def on_bar(stl: STLRuntime, params: dict) -> None:
        ...
        # С долиной берём запас: вырезанный долинный хвост надо заместить, чтобы
        # у сигнала осталось окно в `need` баров (потолок хвоста раннера решает).
        fetch_n = need + DV_FETCH_EXTRA if dv_on else need
        bars = await stl.get_bars(symbol, tf=1, n=fetch_n)
        if len(bars) < need:
            return
        in_dv = False
        sig_bars = bars[-need:]   # ОКНО, не весь хвост (см. урок explain 05.08.2026)
        if dv_on:
            sig_bars, in_dv = dv_sig_window(bars, need, dv_win, dv_pts)
        # Долина глубже запаса съела прогрев -> сигнала нет, робот молчит (hold):
        # желаемое поведение ВНУТРИ долины (входы всё равно закрыты in_dv), а не
        # отказ — TP/стоп/усреднение ниже работают по СЫРЫМ барам. После пробоя
        # sig_bars = сырой хвост целиком, сигнал жив сразу (dv_sig_window).
        want = signal(sig_bars, params) if len(sig_bars) >= need else None   # +1/-1/0/None
...
PARAM_DESC: dict[str, str] = {
    ...
    "dv_bars": "«Долина смерти», окно детектора в барах М1: сколько последних минутных закрытий должно уложиться в узкий коридор, чтобы робот признал боковик. Например 60 = «последний час цена стоит». Работает только в паре с dv_range_pts (оба > 0). В долине робот НЕ открывает новых входов и не добирает (выходы — тейк, стоп, разворотное закрытие — работают всегда), а бары ТЕКУЩЕЙ долины ВЫРЕЗАЮТСЯ из расчёта сигнала: пока боковик длится, сигнал заморожен на до-долинном контексте, а первый же пробойный бар возвращает полную серию — робот снова торгует сразу. Завершённые долины истории не вырезаются. Не подходит стратегиям на дневных уровнях/времени бара (pivot).",
