...
def make_on_bar(rid: str):
    ...
    async def on_bar(stl: STLRuntime, params: dict) -> None:
        ...
        # С долиной берём запас: вырезанные бары надо чем-то заместить, чтобы у
        # сигнала осталось окно в `need` баров (потолок хвоста раннера — 600).
        fetch_n = need + 3 * dv_win if dv_on else need
        ...
        if dv_on:
            dvf = dv_flags(_c(bars), dv_win, dv_pts)
            in_dv = bool(dvf[-1])
            if any(dvf):
                sig_bars = [b for b, f in zip(bars, dvf) if not f][-need:]
        # Глубокая долина съела прогрев -> сигнала нет, робот молчит (hold): это
        # желаемое поведение, а не отказ — TP/стоп/усреднение ниже работают по
        # СЫРЫМ барам, вырезание касается только сигнальной функции.
