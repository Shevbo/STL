...
def create_app() -> FastAPI:
    ...
    async def list_strategies(request: Request):
        ...
        core = [
            ...
            },
            {
                ...
                "description": (
                    "US-Open Range + FVG/Ретест — одна сделка в день от открытия биржи США (9:30 Нью-Йорк:\n"
                    "16:30 МСК при летнем времени США ~март–ноябрь, 17:30 МСК зимой — при переводе часов в США\n"
                    "переключите параметр open_hour).\n"
                    "ICT «Silver Bullet» + фильтр ложных пробоев из видео В. Дудукчанова «Свеча 9:30».\n\n"
                    "КАК РАБОТАЕТ:\n"
                    "1. Первая свеча длиной range_min минут после открытия США задаёт диапазон дня [хай RH, лоу RL]\n"
                    "   (на M1 при range_min=5 и открытии 16:30 это блок 16:30..16:34).\n"
                    "2. После закрытия диапазона на 1-мин графике вход по режиму entry_mode:\n"
                    "   • 0 = FVG+пробой: close за уровнем + Fair Value Gap в ту же сторону (long: low[-1]>high[-3]).\n"
                    "   • 1 = РЕТЕСТ+отвержение (видео): дождаться закрытия за уровнем, ЗАТЕМ возврата к уровню и\n"
                    "     сильной «бритой» свечи-отвержения (тело/размах>=rej_frac), закрывшейся обратно в сторону.\n"
                    "     Отсекает ложные пробои, которые не удерживают уровень на ретесте.\n"
                    "   • 2 = любой из двух.\n"
                    "3. ВХОД ОДИН РАЗ. Стоп = stop_pct% высоты диапазона от края (0=на краю, >0 шире, <0 туже,\n"
                    "   -100=у противоположного края). Тейк = R:R × риск (2:1).\n"
                    "4. Выход: TP, SL или флэт в 23:45 МСК. Повторно до следующего дня НЕ входит.\n\n"
                    "ДЛЯ ЧЕГО: ловит импульс от открытия США. Ретест/FVG отсекают ложные пробои;\n"
                    "жёсткий R:R и один вход в день держат риск под контролем. Все развилки — параметры под свип."
                ),
    ...
    async def robot_stand(robot_id: str, request: Request, agent_id: str | None = None):
        ...
        from trader.lab.robot_stand import CAPS_PAPER, paper_record
        return {"source": "paper", "robot": paper_record(robot_id, live), "live": live,
                "point_value": float(live.get("point_value") or 1) or 1.0,
                "received_at_ms": None, "caps": CAPS_PAPER}
