...
# экран «Чат разработчиков» и получает Telegram; `stl-dev-spare` — подхват,
...
_NO_SLA = ("operator", "stl-dev-spare")
...
AGENTS: dict[str, str] = {
    ...
    # ЧЕТВЁРТОЕ ОКНО — ПОДХВАТ, а не четвёртый работник. Три основных окна живут
    # на одной Windows-VM в квартире, где свет пропадает на 12-24 часа: гаснут
    # они ВСЕ РАЗОМ. Это окно живёт на smain и открывается с ноутбука по ssh.
    # Пока основные тёмные — оно ведёт все три зоны; как только они вернулись —
    # НЕ редактирует их код, а передаёт письмом владельцу. Иначе мы получим
    # четырёхстороннюю гонку в тех же файлах, ради которой зоны и заводились.
    "stl-dev-spare": "ПОДХВАТ на smain (ssh с ноутбука): пока окна на "
                     "Windows-VM без света — ведёт их зоны; когда они живы — "
                     "только читает и пишет письма",
}
_BROADCAST = "all"
