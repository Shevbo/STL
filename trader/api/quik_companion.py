...
_CONTROL_RULES: list[tuple[str, str, bool]] = [
    ...
    # Перекладка контрактов (docs/design/expiry-roll.md): кампания НИКОГДА не
    # закрывает позицию сама, она только показывает, что робот не вышел. Значит
    # любая её тревога ждёт человека, и «исправится автоматически» тут было бы враньём.
    ("экспирация", f"{_MANUAL}: открой экран Экспирация в STL", True),
...
async def snapshot(request: Request, agent_id: str | None = None, bars: int = 30):
        ...
        if cur_mode == "real":
            ...
            if tail:
                ...
                chart = {
                    ...
                    # ВЫХОД БЕЗ ЦЕНОВОГО УРОВНЯ. Закрытие по смене сигнала уходит
                    # по рынку, поэтому в `planned` (там только уровни) его нет и
                    # быть не должно. Но позиция с таким выходом не «без выхода», и
                    # панель обязана это сказать строкой (оператор 26.09.2026).
                    # EMA СТРАТЕГИИ — ТОЛЬКО ПРИСЛАННЫЕ. Их считает раннер тем же
                    # кодом и на том же окне ПРОГРЕВА, что и стратегия (до 560
                    # баров у живого 2EMA). Пересчитать их по видимому хвосту
    ...
    # counts_active — то же самое, но только по ЖИВЫМ: с выключенным фильтром
    # «исполненные и снятые» панель считала видимые строки сама и повторяла
    # ошибку обрезки.
