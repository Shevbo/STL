...
ЧАСОВЫЕ бары, а не дневные: детектору нужно не меньше 60 точек на окно, иначе порог
ER задран шумовым полом и ответ решается третьим знаком (двухнедельный рост RIU6
17-30.07 по дневным барам классифицировался «ростом» с ER 0.713 при пороге 0.696,
а по часовым и минутным — боковиком). Часы и минуты между собой сходятся.
...
async def main():
    ...
    for sym in CONTRACTS:
        try:
            bars = await load_bars_iss(sym, date(2025, 9, 1), date(2026, 8, 17), 60)
        ...
        rows = []
        for w in WEEKS:
            n = w * 5 * 14                 # часовых баров в окне (14 торговых часов в дне)
            if len(bars) < n:
                continue
            step = max(1, n // 4)
