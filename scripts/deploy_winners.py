...
async def main():
    ...
    ap.add_argument("--strategies", default="fvg,order_block,pivot_reversal",
                    help="comma list of strategy ids to deploy winners for")
    ap.add_argument("--campaign", default="", help="campaign_run prefix (default: latest)")
    ...
    args = ap.parse_args()
    want = [x.strip() for x in args.strategies.split(",") if x.strip()]
    ...
    async with pool.acquire() as c:
        ...
        camp = args.campaign
        if not camp:
            camp = await c.fetchval(
                "SELECT campaign_run FROM optimization_leaderboard "
                "WHERE strategy = ANY($1::text[]) ORDER BY created_at DESC NULLS LAST LIMIT 1",
                want,
            )
        if not camp:
            print("no campaign found for", want); return
