...
# СКЛЕЙКА, а не контракт. GZU6 в январе-мае — мёртвый дальний контракт (627 баров и
# 3 935 контрактов за январь против 24 338 баров и 9.5 млн в июле), прогон по нему за
# полгода наполовину считался по пустой книге. Базовый код 'GZ' — непрерывная серия по
# переднему контракту, бэктест перестаёт упираться в экспирацию.
SYMBOL = "GZ"
STRAT = "williams_r"
CAMPAIGN = "wrgznight"           # ОДИН слуг на всю ночь: иначе каждое задание = своя кампания
...
DEADLINE_SEC = 5 * 3600          # общий предохранитель: отчёт обязан быть к утру
JOB_TIMEOUT_SEC = 90 * 60        # одно задание дольше полутора часов = что-то не так
# Делим окно на два задания вместо одного: прогресс видно вдвое раньше, и каждое
# заведомо укладывается в JOB_TIMEOUT_SEC даже если i9 окажется вдвое медленнее оценки.
CHUNK = 1300

# Три НЕЗАВИСИМЫХ окна по 5 месяцев на склейке. Третье содержит период, на котором
# живой бумажный робот показал плюс. Склейка в кэше STL покрывает 2024-08…2026-07.
WINDOWS = [
    ("w1", "2025-05-01", "2025-09-30"),
    ("w2", "2025-10-01", "2026-02-28"),
    ("w3", "2026-03-01", "2026-08-02"),
]
...
# Размер сетки посчитан по замеру: 1 комбинация на 2 годах склейки = 136 с ВМЕСТЕ с
# выкачкой баров; чистый счёт ~4.5 ядро-с на 57k баров. Окно 5 месяцев ≈ 80k баров ->
# при 6 воркерах (оператор просил не греть i9) ~1 комбинация/с. 2592 × 3 окна ≈ 2 часа.
AXES_COARSE = {
    "period": [10, 16, 22, 28, 34, 40],
