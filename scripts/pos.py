...
def main() -> int:
    ...
    # KILL-SWITCH АГЕНТА — так же в глаза, как шапка СВЕЖО/СТАРО. 26.09.2026 агент
    # час отклонял каждую заявку обоих реальных роботов, закрывавших шорты, а STL
    # считал торговлю разрешённой; час простоя нашёл человек в логе раннера на VDS.
    blocked = t.get("agent_blocked")
    lim_age = int(t.get("limits_age_ms") or -1)
    if blocked is True:
        print("АГЕНТ БЛОКИРУЕТ ТОРГОВЛЮ: заявки роботов отклоняются (kill-switch на "
              "агенте; снимается ТОЛЬКО перезапуском агента, эхо лимитов "
              f"{lim_age/1000:.0f} с назад)")
    elif blocked is None:
        print("  kill-switch агента НЕИЗВЕСТЕН: эха лимитов нет (агент старее релиза "
              "поля) — «торговля разрешена» из этого НЕ следует")
    feed = t.get("feed") or []
    if feed:
        print("лента: " + "; ".join(
            "%s %s (бид %s аск %s, %s с)" % (
