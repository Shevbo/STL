...
def run_model(prompt: str, spec: dict, cwd: Path, timeout_s: int) -> tuple[str, int, str]:
    ...
    # Коэффициент 1/10 на чтение кэша — МОЯ оценка, а не факт вендора. Сам CLI
    # отдаёт total_cost_usd: это тоже оценка на стороне клиента и может разойтись
    # со счётом, но она хотя бы вендорская и сразу в деньгах. Пишем обе цифры и
    # не притворяемся, что знаем точную: невидимый расход опаснее большого, а
    # выдуманно точный — опаснее честно приблизительного.
    usd = d.get("total_cost_usd")
    money = f", по оценке CLI ${usd:.4f}" if isinstance(usd, (int, float)) else ""
    tail = (f"\n\nРАСХОД: ввод {inp:,}, вывод {out:,}, запись кэша {cw:,}, "
            f"чтение кэша {cr:,} -> к бюджету {spent:,}{money}")
