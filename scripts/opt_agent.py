...
class Agent:
    ...
    async def _bars_for(self, client: httpx.AsyncClient, symbol: str, d_from, d_to) -> list[dict]:
        ...
        if rows:
            # Кэш годится ТОЛЬКО если покрывает ОБА КРАЯ запрошенного окна. Иначе
            # прогон молча посчитается по укороченной истории — та же болезнь, что
            # чинили в STL для agent_bars (запрос «с 01.04» при кэше с 11.05).
            # Про НАЧАЛО знали, про КОНЕЦ — нет: кэш RI кончался 31.07.2026, и всё,
            # что просили до сентября, обрезалось по этот конец незаметно — задание
            # отвечало цифрой по семи неделям вместо девяти. Нашлось на разборе
            # расхождения i9 с хостером в +88% на одной и той же строке (10.09.2026).
            # Допуск 3 дня на выходные/праздники у каждой границы.
            head_ok = rows[0][0] <= lo + 3 * 86400
            tail_ok = max(x[0] for x in rows) >= hi - 3 * 86400
            if head_ok and tail_ok:
                ...
                _log(f"bars: кэш {symbol} не покрывает окно "
                     f"({'начало' if not head_ok else 'конец'} мимо) — беру ISS")
