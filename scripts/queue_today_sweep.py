...
def draw(rng: random.Random) -> dict:
    ...
    if not p.get("sl_pct") and not p.get("sl_frac"):
        p["sl_pct"] = 100
    # Мартингейл, не поднимающий ПЕРВУЮ ступень, — побитовый дубль k_avg=1.0:
    # объём добора целый, next = int(пред × k + 0.5), и при qty=1 любое k < 1.5
    # даёт тот же контракт. 22.09.2026 такие значения составили ТРЕТЬ сетки на
    # 2880 наборов и перевесили медианы и доли плюсовых. Свернуть в базовое 1.0,
    # дальше дубликат уберёт seen. Проверка постфактум: scripts/grid_degeneracy.py.
    _qty = max(1, int(p.get("qty", 1) or 1))
    if int(_qty * float(p.get("k_avg", 10) or 10) / 10.0 + 0.5) == _qty:
        p["k_avg"] = 10
    return p
