...
Считает по УЖЕ закэшированным барам (agent_bars/*.json), без сети и без i9:
  порог = 2 × тейкерский сбор / цена пункта           (в пунктах на круг)
...
async def main() -> int:
    ...
    for fn in files:
        ...
        rows.append({
            ...
            "fee_rub": fee_rub, "fee_pts": fee_pts, "med_range": med,
            "cost": fee_pts / med,
        })
        await asyncio.sleep(0.2)          # вежливо к ISS

    rows.sort(key=lambda r: r["cost"])
    print(f"{'инстр':8} {'группа':10} {'₽/пункт':>9} {'круг, ₽':>9} {'круг, пт':>9} "
          f"{'размах M1':>10} {'ЦЕНА ОБОРОТА':>13}")
    print("-" * 76)
    for r in rows:
        print(f"{r['sym']:8} {r['group']:10} {r['pv']:9.3f} {r['fee_rub']:9.2f} "
              f"{r['fee_pts']:9.2f} {r['med_range']:10.2f} {r['cost']:12.1f}×")
