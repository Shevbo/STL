...
РУЧНОЙ клиент, вызывается только по слову оператора «почта». Автономного
опроса, автоответа и циклов здесь НЕТ и не будет: ночь 17-18.08 два
автоответчика, подтверждая друг другу подтверждения, сожгли лимит подписки
(792 письма, запуск модели раз в 2.9 минуты). Инцидент 438479e.
...
def main(argv: list[str]) -> int:
    ...
    ap.add_argument("cmd", choices=("send", "poll", "windows"))
