...
async def _async_iter(items):
    """Async iterator helper for mocking WebSocket frame iteration."""
    for item in items:
        yield item
...
def make_fake_ws(recv_messages: list[dict]) -> AsyncMock:
    ...
    ws.send = AsyncMock()
    ws.close = AsyncMock()
    # __aiter__ yields subsequent frames as an async iterator
    encoded = [orjson.dumps(m) for m in recv_messages]
    ws.__aiter__ = MagicMock(return_value=_async_iter(encoded))
