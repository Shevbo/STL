import pytest
from trader.lab.backtest import run_single_backtest, compute_metrics
from trader.lab.commission import commission_for
from trader.lab.runtime import Bar


def make_bars(n=100, trend="up") -> list[Bar]:
    prices = []
    p = 100.0
    for i in range(n):
        p += 0.3 if trend == "up" else -0.3
        prices.append(p)
    return [Bar(time=i*60, open=p-0.1, high=p+0.5, low=p-0.5, close=p, volume=1000)
            for i, p in enumerate(prices)]


# Круги НОЧУЮТ (выход на следующие сутки), и все метки лежат в БУДНЯХ: иначе
# сработали бы скальперская скидка или удвоение выходного дня, и ожидания ниже
# перестали бы совпадать с плоской моделью. Обе поправки пинит
# tests/lab/test_scalper_fee.py, здесь проверяется сам учёт комиссии в парах.
_D = 86400
_WED = 1788336000        # среда 02.09.2026 08:00
_TRADES = [
    {"side": "buy", "price": 100.0, "qty": 1, "time": _WED},
    {"side": "sell", "price": 105.0, "qty": 1, "time": _WED + _D},        # четверг
    {"side": "buy", "price": 103.0, "qty": 1, "time": _WED + _D + 3600},
    {"side": "sell", "price": 108.0, "qty": 1, "time": _WED + 2 * _D},    # пятница
]


def test_compute_metrics_winning():
    # Two +5-point round-trips. Commission is tiny vs the 5-point gross, so both win.
    metrics = compute_metrics(_TRADES, initial_equity=100_000.0)
    assert metrics["total_trades"] == 2
    assert metrics["win_rate"] == pytest.approx(1.0)
    assert metrics["total_return"] > 0


def test_compute_metrics_taker_commission_reduces_pnl():
    # Net profit = 10 gross points − the taker commission on all four fills.
    pv = 1.0
    fees = sum(commission_for("RIM6", t["price"], t["qty"], pv, taker=True) for t in _TRADES)
    net = compute_metrics(_TRADES, initial_equity=100_000.0, point_value=pv, symbol="RIM6")
    assert net["net_profit"] == pytest.approx(10.0 * pv - fees)
    assert fees > 0


def test_compute_metrics_empty():
    metrics = compute_metrics([], initial_equity=100_000.0)
    assert metrics["total_trades"] == 0
    assert metrics["total_return"] == pytest.approx(0.0)


@pytest.mark.asyncio
async def test_run_single_backtest_ema():
    from trader.lab.strategies import ema_crossover
    bars = make_bars(200, trend="up")
    result = await run_single_backtest(
        strategy_module=ema_crossover,
        bars=bars,
        symbol="TESTSYMBOL",
        params={"symbol": "TESTSYMBOL", "fast_period": 5, "slow_period": 20},
        initial_equity=100_000.0,
    )
    assert "trades" in result
    assert "equity_curve" in result
    assert "sharpe" in result
