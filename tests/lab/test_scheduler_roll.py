"""Проверка авто-ролла бумажного робота на ближний фьючерсный контракт.

Планировщик перед каждым тиком переводит бумажного робота с истёкшего контракта на
текущий ближний: символ меняется, позиция обнуляется, состояние стратегии начинается
заново. Тесты пинят, что реальный робот при этом НЕ переезжает сам (перекладка живых
денег — решение человека) и что подмена символа доходит до params_json. Термины: ролл,
перекладка контракта, экспирация, ближний контракт, бумажный робот.
"""
import types
import pytest
